...
static void compute_hyperperiod(ts_t* cur_ts) {
    ...
    while (multi[id_max] / multi[id_min] - 1 > EPS) {
        ...
        if (multi[id_min] > multi[id_max]) id_max = id_min;
        for (i = 0; i < N; i++) {
            if (multi[i] < multi[id_min]) id_min = i;
        }
    }
    H = multi[id_min];
...
static void store_max_dl_phi(ts_t* cur_ts) {
    ...
    cur_ts->max_o = Phi(0);
    for (i = 1; i < N; i++) {
        if (D(i) > cur_ts->max_d) cur_ts->max_d = D(i);
        if (Phi(i) > cur_ts->max_o) cur_ts->max_o = Phi(i);
    }
}
...
void ts_rand(ts_t* cur_ts, const ts_rand_t* settings) {
    ...
    switch (settings->per_m) {
        ...
        case per_unif:
            ...
            for (i = 0; i < N; i++) {
                T(i) = floor(settings->per_min + (double)rand() *
                                                     (settings->per_max - settings->per_min) /
                                                     (double)RAND_MAX);
                D(i) = T(i) * (settings->norm_dl_avg - settings->norm_dl_var +
